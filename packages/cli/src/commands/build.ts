...
export async function runBuild(
  ...
  { overwriteSchema = false, explain = false }: { overwriteSchema?: boolean; explain?: boolean } = {}
): Promise<void> {
  // 1. Read config
  let config;
  try {
  ...
  // 4a. --explain: print generated Prisma schema before writing
  if (explain) {
    const prismaFile = files.find(f => f.path === "prisma/schema.prisma" && f.projectRoot === true);
    if (prismaFile !== undefined) {
      console.log("\n── strux --explain: generated prisma/schema.prisma ──\n");
      console.log(prismaFile.content);
      console.log("────────────────────────────────────────────────────\n");
    } else {
      console.log("strux --explain: no Prisma schema generated (no @type records in source)");
    }
  }
  ...
  console.log(
    `strux: ✓ built ${writtenCount} files → ${pkg.outputDir}/`
  );
}
