...
import { isTier2Rod, TIER2_ROD_TYPES } from "./rods/index.js";
...
function emitSummary(ast: TopLevelNode[]): void {
  ...
  for (const node of ast) {
    ...
    for (const rod of rods) {
      if (isTier2Rod(rod.rodType)) { tier2Count++; panelHasStub = true; }
      else if (!TIER2_ROD_TYPES.has(rod.rodType)) tier1Count++;
    }
